    ...
    <div>
      <el-form ref="para-form" :model="paraPacket" class="register-container" label-width="200px">
        ...
        <el-form-item label="噪声类型：">
          <el-select v-model="paraPacket.para1" placeholder="请选择噪声类型">
            <el-option label="高斯噪声" value="1"></el-option>
            <el-option label="椒盐噪声" value="2"></el-option>
          </el-select>
        </el-form-item>
        <div v-if="paraPacket.para1 === '1'">
          <el-form-item label="均值：">
            <el-slider v-model="paraPacket.para2" show-input :min="-1" :max="1" :step="0.1"></el-slider>
          </el-form-item>
          <el-form-item label="方差：">
            <el-slider v-model="paraPacket.para3" show-input :max="1" :step="0.1"></el-slider>
          </el-form-item>
        </div>
        <div v-if="paraPacket.para1 === '2'">
          <el-form-item label="椒噪声阈值：">
            <el-slider v-model="paraPacket.para2" show-input :max="1" :step="0.1"></el-slider>
          </el-form-item>
          <el-form-item label="盐噪声阈值：">
            <el-slider v-model="paraPacket.para3" show-input :max="1" :step="0.1"></el-slider>
          </el-form-item>
        </div>
      </el-form>
...
export default {
  ...
  data(){
    return{
      paraPacket: {
        ...
        stype: '3051',
        img1: 'img1', img2: '',
        para1: '', para2: 0.1, para3: 0.9, para4: '', para5: '', para6: '', para7: '', para8: '',
        status: '处理中'
      },
  ...
  methods: {
    ...
    async post_para() {
      ...
      // post parameter
      await this.$axios
          .post('/add_noise',this.paraPacket)
          .then((res) => {
            console.log(res)
